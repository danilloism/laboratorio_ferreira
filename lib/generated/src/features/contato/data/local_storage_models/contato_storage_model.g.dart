...
const ContatoStorageModelSchema = CollectionSchema(
  ...
  links: {},
  embeddedSchemas: {r'AccountStorageModel': AccountStorageModelSchema},
...
int _contatoStorageModelEstimateSize(
  ...
  var bytesCount = offsets.last;
  {
    ...
    if (value != null) {
      bytesCount += 3 +
          AccountStorageModelSchema.estimateSize(
              value, allOffsets[AccountStorageModel]!, allOffsets);
    }
  }
...
void _contatoStorageModelSerialize(
  ...
  writer.writeObject<AccountStorageModel>(
    offsets[0],
    allOffsets,
    AccountStorageModelSchema.serialize,
    object.account,
  );
  writer.writeBool(offsets[1], object.ativo);
  writer.writeDateTime(offsets[2], object.atualizadoEm);
  writer.writeByteList(offsets[3], object.categorias);
  writer.writeDateTime(offsets[4], object.criadoEm);
  writer.writeString(offsets[5], object.nome);
  writer.writeStringList(offsets[6], object.telefones);
...
ContatoStorageModel _contatoStorageModelDeserialize(
  ...
  object.account = reader.readObjectOrNull<AccountStorageModel>(
    offsets[0],
    AccountStorageModelSchema.deserialize,
    allOffsets,
  );
  object.ativo = reader.readBoolOrNull(offsets[1]);
  object.atualizadoEm = reader.readDateTimeOrNull(offsets[2]);
  object.categorias = reader.readByteList(offsets[3]) ?? [];
  object.criadoEm = reader.readDateTimeOrNull(offsets[4]);
  object.localId = id;
  object.nome = reader.readString(offsets[5]);
  object.telefones = reader.readStringList(offsets[6]) ?? [];
...
P _contatoStorageModelDeserializeProp<P>(
  ...
  switch (propertyId) {
    case 0:
      return (reader.readObjectOrNull<AccountStorageModel>(
        offset,
        AccountStorageModelSchema.deserialize,
        allOffsets,
      )) as P;
...
extension ContatoStorageModelQueryProperty
  ...
  QueryBuilder<ContatoStorageModel, AccountStorageModel?, QQueryOperations>
      accountProperty() {
    return QueryBuilder.apply(this, (query) {
